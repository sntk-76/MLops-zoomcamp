...
def train_and_log_model(data_path, params):
    ...
    with mlflow.start_run():
        ...
        # ✅ Log model artifact so it can be registered
...
def run_register_model(data_path: str, top_n: int):
    ...
    # Get top N runs from the hyperopt experiment
    ...
    # Train and log each of the top N models on test set
    for run in hpo_runs:
        train_and_log_model(data_path=data_path, params=run.data.params)

    # Select the best model from the new experiment
    ...
    # Print the best RMSE for confirmation
    print(f"✅ Best test RMSE: {best_run.data.metrics['test_rmse']}")
    print(f"✅ Registered model: {MODEL_NAME} from run ID: {best_run.info.run_id}")
